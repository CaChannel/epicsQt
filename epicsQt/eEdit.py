...
class eEdit(Qt.QLineEdit):
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Qt.Key_Return or event.key() == Qt.Qt.Key_Enter:
            if self.validator() is not None \
                    and self.validator().validate(self.text(), 0)[0] != Qt.QValidator.Acceptable:
    ...
    def controlInfo(self):
        ...
        if self.pv.field_type() == ca.DBF_SHORT or\
           self.pv.field_type() == ca.DBF_INT or \
           self.pv.field_type() == ca.DBF_LONG:
            self.setValidator(Qt.QIntValidator(lolim, uplim, self))

        if self.pv.field_type() == ca.DBF_ENUM:
            self.setValidator(Qt.QIntValidator(0, self.pv.pv_nostrings-1, self))

        if self.pv.field_type() == ca.DBF_FLOAT or self.pv.field_type() == ca.DBF_DOUBLE:
    ...
    def valueChanged(self):
        ...
        if self.pv.field_type() == ca.DBF_DOUBLE or \
           self.pv.field_type() == ca.DBF_FLOAT:
            if hasattr(self.pv, 'pv_precision'): format = '%%.%df' % self.pv.pv_precision
            else: format = '%f'
            self.setText(format % self.pv.pv_value)
        elif self.pv.field_type() == ca.DBF_CHAR:
